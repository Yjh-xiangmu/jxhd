<template>
  <div class="min-h-screen bg-[#FDFCF8] flex flex-col items-center justify-center p-6 font-sans">
    ...
    <div class="text-center max-w-4xl text-[#333322] mb-10">
    ...
    <div class="bg-white p-10 rounded-[32px] border border-[#EFE9E1] shadow-sm hover:shadow-md transition-shadow w-full max-w-md">
      ...
      <div class="space-y-6">
        ...
        <button @click="handleLogin" :disabled="loading"
          class="w-full h-14 bg-[#5A5A40] text-white rounded-2xl mt-4 hover:bg-[#4A4A35] transition-all duration-300 font-bold text-lg tracking-widest flex items-center justify-center group disabled:opacity-60 disabled:cursor-not-allowed">
          <span v-if="loading">登录中...</span>
          <template v-else>
            进 入 系 统
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 ml-2 group-hover:translate-x-2 transition-transform duration-300"
...
const handleLogin = async () => {
  ...
  try {
    ...
    else if (user.role === 'teacher') router.push('/teacher/students')
    else if (user.role === 'parent')  router.push('/parent/timeline')
